...
def catalogs():
    ...
    categories = session.query(Category).outerjoin(stmt, 
...
def category(category_name):
    ...
    return render_template('category.html', categoryList=categories, c=category, 
        categoryItems=category_items, user_name=login_session['username'])
...
def addItem():
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('catalogs', categoryList=categories, 
            latestItems=latest_items, user_name=login_session['username']))
    else:
        stmt = session.query(Item.category_id, func.count(
            '*').label('item_counts')).group_by(Item.category_id).subquery()
        categories = session.query(Category).outerjoin(stmt, 
...
def editItem(item_name):
    ...
    if request.method == 'POST':
        ...
        categories = session.query(Category).outerjoin(stmt, 
            Category.id == stmt.c.category_id).filter(
            stmt.c.item_counts >= 0).order_by(Category.id)
        return render_template('editItem.html',  categoryList=categories, 
            item=editedItem, user_name=login_session['username'])
...
def itemDetail(category_name, item_name):
    ...
    if ('username' not in login_session or creator.id != login_session['user_id']):
        ...
        return render_template('item.html', item=item, user_name=login_session['username'])
